...
/* Blurred dashboard preview — abstract bar charts and KPI shapes suggest data */
function DashboardPreview() {
  const bars = [45, 72, 38, 90, 61, 55, 83, 42, 68, 77, 50, 65];
  const kpis = ["Active Tenants", "Platform Revenue", "Total Bookings", "This Month"];
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none select-none" aria-hidden>
      {/* Sidebar silhouette */}
      <div className="absolute left-0 top-0 bottom-0 w-52 bg-[#111C44]/60 border-r border-white/[0.04]">
      ...
      <div className="absolute left-52 top-0 right-0 bottom-0 p-6 pt-14">
        {/* KPI cards row */}
        <div className="grid grid-cols-4 gap-3 mb-6">
          {kpis.map((label, i) => (
            <div key={i} className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-3">
              <div className="w-6 h-6 rounded-lg bg-[#C9A84C]/20 mb-2" />
              <div className="h-5 w-12 rounded bg-white/[0.08] mb-1.5" />
              <div className="h-2 rounded bg-white/[0.04]" style={{ width: "70%" }} />
            </div>
          ))}
        </div>
          ...
          <div className="flex items-end gap-2 h-20">
            {bars.map((h, i) => (
              <div
                ...
                style={{
                  height: `${h}%`,
                  background: i === 4 ? "rgba(201,168,76,0.4)" : "rgba(255,255,255,0.06)",
                }}
              />
            ))}
          </div>
...
export default function SuperAdminLogin({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#0B1437] flex items-center justify-center p-4 relative overflow-hidden">
      {/* Ambient blobs */}
      <div className="absolute -top-40 -left-40 w-[600px] h-[600px] rounded-full bg-[#C9A84C]/[0.06] blur-[120px] pointer-events-none" />
      <div className="absolute -bottom-40 -right-40 w-[600px] h-[600px] rounded-full bg-[#4318FF]/[0.08] blur-[120px] pointer-events-none" />

      {/* Blurred dashboard preview — Zeigarnik Effect */}
      <DashboardPreview />
      <div className="absolute inset-0 backdrop-blur-md bg-[#0B1437]/70 pointer-events-none" />

      <div className="relative w-full max-w-sm z-10">
        {/* Brand */}
        <div className="flex flex-col items-center mb-8">
          <div className="sa-logo-shimmer w-16 h-16 rounded-2xl overflow-hidden mb-4 shadow-2xl shadow-[#C9A84C]/10 border border-[#C9A84C]/20">
        ...
        {stage === "request" ? (
          <form
            ...
            className="bg-[#111C44]/80 backdrop-blur-xl rounded-3xl border border-white/[0.08] p-7 shadow-2xl shadow-black/40"
          >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#C9A84C] to-[#E8C96A] text-[#0B1437] font-bold text-sm tracking-wide hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-[#C9A84C]/20"
            >
              {loading
          ...
          <div className="bg-[#111C44]/80 backdrop-blur-xl rounded-3xl border border-white/[0.08] p-7 shadow-2xl shadow-black/40 flex flex-col items-center text-center">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#01B574] to-[#3DDB85] flex items-center justify-center shadow-lg shadow-[#01B574]/30 mb-4">
              <CheckCircle2 className="sa-check-pop w-7 h-7 text-white" />
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-[#C9A84C] hover:text-white transition-colors"
            >
