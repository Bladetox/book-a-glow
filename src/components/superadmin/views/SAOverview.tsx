...
export default function SAOverview() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from("bookings").select("id", { count: "exact", head: true }).gte("created_at", new Date(new Date().setHours(0,0,0,0)).toISOString()),
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20"><Loader2 className="w-5 h-5 animate-spin" style={{ color: "#00c853" }} /></div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {kpis.map(({ label, value, icon: Icon, accent }) => (
            <GlassCard key={label} className="p-5">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center mb-4" style={{ background: `${accent}18`, border: `1px solid ${accent}30` }}>
