...
function TenantDrawer({ tenant, onClose, onToggleActive, onPlanChanged }: {
  ...
  const handlePlanSave = async () => {
    ...
    await saLog("tenant.plan_changed", "tenant", tenant.id, tenant.name, {
      from: tenant.stats.plan || "starter",
      to:   selectedPlan,
    });
...
export default function SAOverview() {
  ...
  const fetchTenants = async () => {
    ...
    setTenants(rows.map(t => ({ ...t, plan: planMap[t.id] ?? "starter" })));
    setLoading(false);
  };
    ...
    <div className="space-y-5 max-w-6xl">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div>
          ...
          <p className="text-white/40 text-sm">{activeCount} active · {inactiveCount} suspended · {tenants.length} total</p>
        </div>
