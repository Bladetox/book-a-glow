...
// Plan prices sourced from src/pages/Pricing.tsx — the single source of truth
// Starter R399 | Professional R599 | Studio R899 | Enterprise = custom (contact sales)
const PLAN_PRICE_FIELDS: SettingField[] = [
  { key: "plan_price_starter",      label: "Starter (R/mo)",       type: "number", placeholder: "399" },
  { key: "plan_price_professional", label: "Professional (R/mo)",  type: "number", placeholder: "599" },
  { key: "plan_price_studio",       label: "Studio (R/mo)",        type: "number", placeholder: "899" },
  { key: "plan_price_enterprise",   label: "Enterprise (R/mo)",    type: "number", placeholder: "Custom — contact sales" },
];
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-start justify-between">
        ...
        <button onClick={fetchSettings} className="p-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white/40 hover:text-white/70 transition-colors">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="bg-[hsl(220,13%,7%)] border border-white/[0.06] rounded-2xl p-5 space-y-4">
            <div>
              ...
              <p className="text-[11px] text-white/25 mt-1">Monthly prices in rands · matches nextslot.co.za/pricing</p>
            </div>
      ...
      <div>
        <h3 className="text-white/70 font-semibold text-sm mb-3 flex items-center gap-2">
          <KeyRound className="w-4 h-4 text-white/30" /> Change Password
        </h3>
        <form onSubmit={handlePasswordChange} className="bg-[hsl(220,13%,7%)] border border-white/[0.06] rounded-2xl p-5 space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs text-white/50 font-medium">New Password</label>
            <input type="password" value={newPw} onChange={e => { setNewPw(e.target.value); setPwError(""); }} placeholder="Min 6 characters" required
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-violet-500/50 transition-colors" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs text-white/50 font-medium">Confirm New Password</label>
            <input type="password" value={confirmPw} onChange={e => { setConfirmPw(e.target.value); setPwError(""); }} placeholder="Repeat new password" required
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-violet-500/50 transition-colors" />
          </div>
          {pwError && (
            <div className="flex items-start gap-2 text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
          ...
          {pwSuccess && (
            <div className="flex items-center gap-2 text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2">
              <Check className="w-3.5 h-3.5 shrink-0" /> Password updated successfully.
            </div>
          )}
          <button type="submit" disabled={pwLoading}
            ...
            {pwLoading ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Updating…</> : <><KeyRound className="w-3.5 h-3.5" /> Update Password</>}
          </button>
          <p className="text-[11px] text-white/20 flex items-center gap-1.5">
            <Shield className="w-3 h-3" /> No current password required — session must be active.
          </p>
        </form>
      </div>
    </div>
