...
const ACTION_COLOR: Record<string, string> = {
  ...
  "webhook.retry":       "text-amber-400 bg-amber-500/10 border-amber-500/20",
        ...
        <div className="sm:ml-auto flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-2 text-xs bg-white/[0.03] border border-white/[0.07] rounded-xl text-white/60 placeholder:text-white/20 focus:outline-none focus:border-emerald-500/30 w-56"
            />
          </div>
        </div>
      </div>
                ...
                return (
                  <Fragment key={log.id}>
                    <tr onClick={() => setExpanded(isOpen ? null : log.id)}
                      ...
                      <td className="px-4 py-3">
                        {hasMeta ? (
                          <span className="text-[10px] text-emerald-400/60 bg-emerald-500/10 border border-emerald-500/15 px-2 py-0.5 rounded-full">
                            {isOpen ? "▲ hide" : "▼ view"}
                          </span>
                    ...
                    {isOpen && hasMeta && (
                      <tr className="border-b border-white/[0.03] bg-white/[0.01]">
                        <td colSpan={5} className="px-6 py-3">
                          <pre className="text-[11px] text-emerald-400/50 font-mono whitespace-pre-wrap">{JSON.stringify(log.meta, null, 2)}</pre>
                        </td>
                      </tr>
                    )}
                  </Fragment>
