...
import { DollarSign, TrendingUp, CreditCard, ArrowUpRight, ArrowDownRight, BarChart3 } from "lucide-react";
import {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Total Revenue",   value: toRand(totalRev),   sub: "all time",         icon: DollarSign, accent: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/15" },
          { label: "This Month",      value: toRand(thisMonth),  sub: "current month",    icon: TrendingUp, accent: "text-blue-400",    bg: "bg-blue-500/10",    border: "border-blue-500/15" },
          ...
          <GlassCard key={label} className="p-5">
            <div className="flex items-start justify-between mb-4">
              ...
              {label === "This Month" && momPct !== null && (
                <span className={`inline-flex items-center gap-0.5 text-[11px] font-semibold ${momUp ? "text-emerald-400" : "text-red-400"}`}>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={monthlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={2} fill="url(#rev-grad)" dot={{ fill: "#10b981", r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: "#10b981" }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </GlassCard>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={gatewayData} layout="vertical" margin={{ top: 0, right: 4, left: 0, bottom: 0 }}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="revenue" fill="#10b981" fillOpacity={0.7} radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </GlassCard>
            ...
            <thead>
              <tr className="border-b border-white/[0.05]">
                {["Amount","Type","Gateway","Status","Date","Tenant"].map(h => (
                  <th key={h} className="text-left text-[10px] text-white/25 font-semibold uppercase tracking-wider px-4 py-3">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="text-center py-10 text-white/20 text-xs">No payments found</td></tr>
              ) : payments.slice(0, 100).map(p => (
                <tr key={p.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3 text-white/80 font-semibold text-sm tabular-nums">{toRand(p.amount)}</td>
                  <td className="px-4 py-3 text-white/40 text-xs">{p.payment_type || "—"}</td>
                  <td className="px-4 py-3 text-white/40 text-xs">{p.gateway || "—"}</td>
                  ...
                  <td className="px-4 py-3 text-white/25 text-[11px] font-mono">
