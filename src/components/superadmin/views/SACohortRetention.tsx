...
interface BookingRow { tenant_id: string; created_at: string; status: string; }
...
const BOOKING_LIMIT = 5000; // Hard cap — document clearly so future devs understand
...
export default function SACohortRetention() {
  ...
  const load = async () => {
    ...
    const [{ data: tenants }, { data: bookings }] = await Promise.all([
      ...
      // NOTE: capped at BOOKING_LIMIT. If booking volume grows beyond this,
      // this query needs to be moved to an RPC / materialized view server-side.
      supabase.from("bookings")
        .select("tenant_id,created_at,status")
        .not("tenant_id", "is", null)
        // Only count paid/confirmed bookings as retained — not cancelled
    ...
    if (bRows.length === BOOKING_LIMIT) setTruncated(true);
    ...
    const result: CohortMonth[] = keys.map(key => {
      ...
      for (let m = 0; m <= Math.min(monthsElapsed, 11); m++) {
        ...
        if (targetKey > currentKey) {
          ...
          const retained = ids.filter(tid => tenantBookingMonths[tid]?.has(targetKey)).length;
  ...
  const monthHeaders = Array.from({ length: maxMonths }, (_, i) => i === 0 ? "M0" : `M+${i}`);

  const avgRetention: (number | null)[] = monthHeaders.map((_, mi) => {
    const vals = cohorts.map(c => c.retention[mi]).filter((v): v is number => v !== null && v !== undefined);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>
            Monthly signup cohorts · confirmed + complete bookings only
          </p>
        </div>
      ...
      {/* Truncation warning */}
      {truncated && (
        <div className="flex items-center gap-2.5 rounded-xl border border-yellow-500/20 bg-yellow-500/[0.06] px-4 py-3">
          <AlertTriangle className="w-4 h-4 text-yellow-400 shrink-0" />
          <p className="text-xs text-yellow-400/80">
            Booking volume has reached the {BOOKING_LIMIT.toLocaleString()}-row client-side cap.
            Retention data may be incomplete. Move this query to a Supabase RPC for accurate results at this scale.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 text-[10px]">
        ...
        <span className="ml-auto text-[10px]" style={{ color: "rgba(255,255,255,0.2)" }}>
          M0 = signup month · retention = had ≥1 confirmed or complete booking
        </span>
      </div>

      {avgRetention.some(v => v !== null) && (
        <div className="rounded-xl border p-4" style={{ background: "rgba(255,255,255,0.025)", borderColor: "rgba(255,255,255,0.07)" }}>
          ...
          <div className="flex items-end gap-2 overflow-x-auto pb-1">
            {avgRetention.map((v, i) => (
              <div key={i} className="flex flex-col items-center gap-1 min-w-[36px]">
                ...
                <div className="w-6 rounded-sm transition-all"
                  style={{ height: v !== null ? `${Math.max(v * 0.6, 4)}px` : "4px", background: v !== null ? retentionColor(v).bg : "rgba(255,255,255,0.04)", border: `1px solid ${v !== null ? retentionColor(v).text + "44" : "transparent"}` }} />
          ...
          <table className="w-full text-xs">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                <th className="px-5 py-3.5 text-left font-medium sticky left-0 min-w-[100px]" style={{ color: "rgba(255,255,255,0.25)", background: "rgba(12,12,12,0.95)" }}>Cohort</th>
                <th className="px-3 py-3.5 text-right font-medium min-w-[52px]" style={{ color: "rgba(255,255,255,0.25)" }}>Size</th>
                {monthHeaders.map(h => (
                  <th key={h} className="px-2 py-3.5 text-center font-medium min-w-[52px]" style={{ color: "rgba(255,255,255,0.25)" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {cohorts.map(c => (
                <tr key={c.key} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                  <td className="px-5 py-3 font-medium sticky left-0" style={{ color: "rgba(255,255,255,0.6)", background: "rgba(12,12,12,0.95)" }}>{c.label}</td>
                  <td className="px-3 py-3 text-right tabular-nums" style={{ color: "rgba(255,255,255,0.4)" }}>{c.size}</td>
                  {monthHeaders.map((_, mi) => {
                    ...
                    return (
                      <td key={mi} className="px-2 py-3 text-center">
                        <span className="inline-flex items-center justify-center w-10 h-7 rounded-md text-[11px] font-semibold tabular-nums"
              ...
              {cohorts.length === 0 && (
                <tr><td colSpan={monthHeaders.length + 2} className="px-5 py-16 text-center" style={{ color: "rgba(255,255,255,0.2)" }}>
                  No cohort data yet
                </td></tr>
              )}
            </tbody>
