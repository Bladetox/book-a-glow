...
import { X, CalendarCheck, AlertTriangle, MessageCircle, Cake } from "lucide-react";
...
const WaButton = ({ phone, msg }: { phone: string; msg: string }) => (
  <a href={waLink(phone, msg)} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
...
export default function ClientAlertsModal({
  ...
  const { data: settingsRows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("app_settings").select("key, value")
        .eq("tenant_id", tenantId).in("key", SETTING_KEYS);
  ...
  const { Icon, iconColor, accentBorder, accentBg, badgeColor, tipNote, count } = active ?? {
    Icon: CalendarCheck, iconColor: "", accentBorder: "", accentBg: "", badgeColor: "", tipNote: "", count: 0,
  };

  return (
    <AnimatePresence>
      {isOpen && alertType && (
        <motion.div key="alerts-modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/70 backdrop-blur-sm"
          onClick={onClose}>
          <motion.div key="alerts-modal-panel"
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-white/[0.06]">
              ...
              <button onClick={onClose} className="w-7 h-7 rounded-full bg-white/[0.06] flex items-center justify-center text-white/40 hover:text-white/80 transition-colors" aria-label="Close">
                  ...
                  return (
                    <div key={client.id} className={`rounded-xl border ${accentBorder} ${accentBg} px-4 py-3 flex items-center justify-between gap-3`}>
                      ...
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${badgeColor}`}>{client.days_overdue}d overdue</span>
                ...
                inactiveClients.map(client => {
                  ...
                  return (
                    <div key={client.client_id} className={`rounded-xl border ${accentBorder} ${accentBg} px-4 py-3 flex items-center justify-between gap-3`}>
                      ...
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${badgeColor}`}>{client.days_since_booking}d ago</span>
                  ...
                  return (
                    <div key={client.id} className={`rounded-xl border ${accentBorder} ${accentBg} px-4 py-3 flex items-center justify-between gap-3`}>
                      <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                        <p className="text-sm font-semibold text-white/85 truncate">{client.client_name}</p>
                        <p className="text-[11px] text-white/35 truncate">{phone || "—"}</p>
                        {client.label && <p className="text-[10px] text-white/25 italic">{client.label}</p>}
                      </div>
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${badgeColor}`}>
                          {isToday ? "🎂 Today!" : `in ${daysLeft}d`}
                        </span>
                        <span className="text-[10px] text-white/30">{formatOccasionDate(client.occasion_date)}</span>
                        {phone && <WaButton phone={phone} msg={msg} />}
                      </div>
                    </div>
                  );
