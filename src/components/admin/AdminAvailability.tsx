...
const AdminAvailability = () => {
  ...
  useEffect(() => {
    ...
    setWeekAvail((previous) => {
      const fetched = toWeekAvailability(rawSlots);
      const merged = { ...fetched };
      for (const day of dirtyDays.current) if (previous[day]) merged[day] = previous[day];
      return merged;
    });
    setDailyOverrides((previous) => {
      const fetched = toDailyOverrides(rawSlots);
      const merged: DailyOverrides = { ...fetched };
  ...
  const persistDay = useCallback((dayName: string, config: { enabled: boolean; slots: string[] }) => {
    ...
    persistDayTimers.current[dayName] = setTimeout(() => {
      ...
      saveAvailability({ staffId: userId!, dayOfWeek: dayIndex, enabled: config.enabled, slots: normaliseSlots(config.slots), allSlots: ALL_SLOTS }, { onSuccess: () => dirtyDays.current.delete(dayName) });
    }, 500);
  ...
  const getDayConfig = (date: Date): { enabled: boolean; slots: string[]; isOverride: boolean } => {
    ...
    const weekly = weekAvail[format(date, "EEEE")] || { enabled: false, slots: [] };
  ...
  const persistDailyOverride = useCallback((date: Date, config: { enabled: boolean; slots: string[] }) => {
    ...
    persistDailyOverrideTimers.current[iso] = setTimeout(() => {
      ...
      saveDailyOverride({ staffId: userId!, date: iso, dayOfWeek: getDay(date), enabled: config.enabled, slots: normaliseSlots(config.slots), allSlots: ALL_SLOTS }, { onSuccess: () => dirtyDates.current.delete(iso) });
    }, 500);
  ...
  const toggleDailySlot = (date: Date, slot: string) => {
    const iso = format(date, "yyyy-MM-dd");
    const current = getDayConfig(date);
    const newSlots = normaliseSlots(current.slots).includes(slot) ? normaliseSlots(current.slots).filter((s) => s !== slot) : [...normaliseSlots(current.slots), slot].sort();
  ...
  const clearDailyOverride = (date: Date) => {
    const iso = format(date, "yyyy-MM-dd");
    if (persistDailyOverrideTimers.current[iso]) { clearTimeout(persistDailyOverrideTimers.current[iso]); delete persistDailyOverrideTimers.current[iso]; }
    dirtyDates.current.add(iso);
    setDailyOverrides((prev) => { const next = { ...prev }; delete next[iso]; return next; });
    saveDailyOverride({ staffId: userId!, date: iso, dayOfWeek: getDay(date), enabled: false, slots: [], allSlots: [], deleteOnly: true }, { onSuccess: () => dirtyDates.current.delete(iso) });
  };
