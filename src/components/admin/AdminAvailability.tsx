...
const AdminAvailability = () => {
  ...
  const persistDay = useCallback((dayName: string, config: { enabled: boolean; slots: string[] }) => {
    ...
    persistDayTimers.current[dayName] = setTimeout(() => {
      ...
      saveMutation.mutate({ staffId: userId, dayOfWeek: dayIndex, enabled: config.enabled, slots: normaliseSlots(config.slots), allSlots: ALL_SLOTS });
    }, 500);
  }, [userId, saveMutation]);
  ...
  const toggleDayEnabled = (day: string) => {
    setWeekAvail((prev) => {
      ...
      const next = { ...prev, [day]: { enabled, slots: enabled && current.slots.length === 0 ? [...ALL_SLOTS] : normaliseSlots(current.slots) } };
  ...
  const getDayConfig = (date: Date): { enabled: boolean; slots: string[]; isOverride: boolean } => {
    ...
    if (dailyOverrides[iso]) return { ...dailyOverrides[iso], isOverride: true };
  ...
  const persistDailyOverride = useCallback((date: Date, config: { enabled: boolean; slots: string[] }) => {
    ...
    persistDailyOverrideTimers.current[iso] = setTimeout(() => {
      ...
      saveDailyMutation.mutate({ staffId: userId, date: iso, dayOfWeek, enabled: config.enabled, slots: normaliseSlots(config.slots), allSlots: ALL_SLOTS });
    }, 500);
  ...
  const toggleDailyEnabled = (date: Date) => {
    ...
    const next = { enabled: !current.enabled, slots: !current.enabled && current.slots.length === 0 ? [...ALL_SLOTS] : normaliseSlots(current.slots) };
  ...
  const clearDailyOverride = (date: Date) => {
    ...
    saveDailyMutation.mutate({ staffId: userId, date: iso, dayOfWeek: getDay(date), enabled: false, slots: [], allSlots: [], deleteOnly: true });
  };
