...
import {
  ...
  Users, CalendarCheck, Send, ChevronDown, Info, Trash2, Bot,
} from "lucide-react";
...
const AdminLoyalty = () => {
  ...
  // Nexty outside-settings candidates: repeat clients who book services OTHER than
  // the qualifying service. Fetched directly from DB via RPC - data only, no fabrication.
  const { data: nextyOutsideRaw = [] } = useQuery({
    queryKey: ["nexty-outside-candidates", tenantId],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_nexty_outside_candidates", { p_tenant_id: tenantId });
      if (error) throw error;
      return (data ?? []) as {
        client_name: string; phone: string; booking_count: number;
        total_spend: number; avg_spend: number;
        last_booking_date: string; days_since_last: number;
      }[];
    },
    staleTime: 5 * 60 * 1000,
  });

  const nextyOutsideCandidates = useMemo(() =>
    nextyOutsideRaw
      .filter(c => !enrollSaved.includes(c.client_name + c.phone))
      .map(c => ({
        ...
        daysSinceLastBooking: c.days_since_last,
        nextDueDate:         "",
      } as EnrollCandidate)),
  [nextyOutsideRaw, enrollSaved]);
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === "Tracker" && (
          <motion.div key="tracker" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex flex-col gap-4">
            ...
            <AnimatePresence>
              {!isLoading && nextyOutsideCandidates.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                  className="rounded-2xl border border-amber-400/25 bg-amber-400/[0.04] p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <Bot className="w-3.5 h-3.5 text-amber-400" />
                    <p className="text-[10px] font-semibold tracking-[0.15em] uppercase text-amber-400/90">
                      Nexty Recommends Enrolling ({nextyOutsideCandidates.length})
                    </p>
                  </div>
                  <p className="text-[11px] text-white/35 mb-3 leading-relaxed">
                    These clients book regularly but fall outside your current loyalty programme settings. Enroll them to start tracking and retain them.
                  </p>
                  <div className="flex flex-col gap-2">
                    {nextyOutsideCandidates.map(c => (
                      <div key={c.client_name + c.phone}
                        ...
                        <div className="flex items-center gap-1.5 shrink-0 mt-1 sm:mt-0">
                          {c.phone && (
                            <WaButton name={c.client_name} status="TIME TO BOOK" phone={c.phone}
                              businessName={settings.businessName} serviceLabel={settings.serviceLabel}
                          ...
                          <button onClick={() => setEnrolling(c)}
                            className="flex items-center gap-1 px-2 py-1.5 rounded-lg bg-amber-400/15 border border-amber-400/30 text-[11px] font-semibold text-amber-300 hover:bg-amber-400/25 transition-colors whitespace-nowrap">
