...
const AdminLoyalty = () => {
  ...
  const counts: Record<FilterType, number> = {
    All: rows.length,
    "On Track": rows.filter((r) => (r.status || "").toUpperCase() === "ON TRACK").length,
    "Time to Book": rows.filter((r) => (r.status || "").toUpperCase() === "TIME TO BOOK").length,
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Total Clients",  value: String(counts.All),            color: "" },
          { label: "On Track",       value: String(counts["On Track"]),    color: "text-emerald-400" },
          { label: "Time to Book",   value: String(counts["Time to Book"]),color: "text-amber-400" },
                ...
                <tr key={r.id} className="border-t border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-3 sm:px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
                      statusColors[(r.status || "").toUpperCase()] || "bg-white/[0.06] text-white/50"
                    }`}>
                      {r.status || "—"}
                    </span>
                  </td>
