...
// ─── InlineClientEditor ─── const InlineClientEditor = ({ rowId, currentName, currentPhone, tenantId, onUpdated }: {   rowId: string; currentName: string; currentPhone: string | null; tenantId: string; onUpdated: (newName: string, newPhone: string) => void; }) => {   const [editing, setEditing] = useState(false);   const [name, setName] = useState(currentName);   const [phone, setPhone] = useState(currentPhone ?? "");   const [saving, setSaving] = useState(false);   const save = async () => {     if (!name.trim()) return;     setSaving(true);     const { error } = await supabase.from("loyalty_tracker").update({ client_name: name.trim(), phone: phone.trim() || null, updated_at: new Date().toISOString() }).eq("id", rowId).eq("tenant_id", tenantId);     setSaving(false);     if (error) toast.error("Failed to update client details");     else { toast.success("Client updated — enrichment will refresh automatically"); setEditing(false); onUpdated(name.trim(), phone.trim()); }   };   if (!editing) return (     <button onClick={e => { e.stopPropagation(); setName(currentName); setPhone(currentPhone ?? ""); setEditing(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors border border-white/[0.06]" title="Edit client name / phone">       <Pencil className="w-3 h-3" /> Edit details     </button>   );   return (     <div className="flex flex-col gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]" onClick={e => e.stopPropagation()}>       <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-white/30">Edit Client Details</p>       <div className="flex flex-col gap-1.5">         <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Full name" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />         <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone (e.g. 0821234567)" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />       </div>       <p className="text-[10px] text-amber-400/70 leading-relaxed">💡 Updating the name to match bookings (e.g. "Amy" → "Amy Damons") fixes live date matching automatically.</p>       <div className="flex gap-2">         <button onClick={save} disabled={saving || !name.trim()} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-[11px] font-semibold text-emerald-400 hover:bg-emerald-500/25 transition-colors disabled:opacity-40">           {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} {saving ? "Saving…" : "Save"}         </button>         <button onClick={() => setEditing(false)} className="px-2.5 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>       </div>     </div>   ); };  // ─── Inli// ─── UnregisterButton ─── const UnregisterButton = ({ rowId, clientName, tenantId, onUnregister }: { rowId: string; clientName: string; tenantId: string; onUnregister: () => void; }) => {   const [confirm, setConfirm] = useState(false);   const [deleting, setDeleting] = useState(false);   const doDelete = async () => {     setDeleting(true);     const { error } = await supabase.from("loyalty_tracker").delete().eq("id", rowId).eq("tenant_id", tenantId);     if (error) { toast.error("Failed to remove client"); setDeleting(false); setConfirm(false); }     else { toast.success(`${clientName} removed from loyalty tracker`); onUnregister(); }   };   if (!confirm) return (     <button onClick={e => { e.stopPropagation(); setConfirm(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-red-400/50 hover:text-red-400 hover:bg-red-500/[0.07] transition-colors border border-red-500/[0.12]" title="Remove from loyalty tracker">       <Trash2 className="w-3 h-3" /> Unregister     </button>   );   return (     <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-500/[0.07] border border-red-500/20" onClick={e => e.stopPropagation()}>       <p className="text-[10px] text-red-400/80 flex-1">Remove <span className="font-semibold">{clientName}</span> from loyalty tracker?</p>       <button onClick={doDelete} disabled={deleting} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-[11px] font-semibold text-red-400 hover:bg-red-500/30 transition-colors disabled:opacity-40">         {deleting ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />} {deleting ? "Removing…" : "Yes, remove"}       </button>       <button onClick={() => setConfirm(false)} className="px-2 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>     </div>   ); };  // ─── EnrollModal ───<InlineNotesEditor rowId={r.id} current={r.notes} tenantId={tenantId} onUpdated={onUpdated} /><InlineClientEditor rowId={r.id} currentName={r.client_name} currentPhone={r.phone} tenantId={tenantId} onUpdated={(newName, newPhone) => { onUpdated(); }} /><UnregisterButton rowId={r.id} clientName={r.client_name} tenantId={tenantId} onUnregister={onUnregister} />optimisticStatus, onOptimisticStatus, onUnregister,onOptimisticStatus: (s: string) => void; onUnregister: () => void;neClientEditor ─── const InlineClientEditor = ({ rowId, currentName, currentPhone, tenantId, onUpdated }: {   rowId: string; currentName: string; currentPhone: string | null; tenantId: string; onUpdated: (newName: string, newPhone: string) => void; }) => {   const [editing, setEditing] = useState(false);   const [name, setName] = useState(currentName);   const [phone, setPhone] = useState(currentPhone ?? "");   const [saving, setSaving] = useState(false);   const save = async () => {     if (!name.trim()) return;     setSaving(true);     const { error } = await supabase.from("loyalty_tracker").update({ client_name: name.trim(), phone: phone.trim() || null, updated_at: new Date().toISOString() }).eq("id", rowId).eq("tenant_id", tenantId);     setSaving(false);     if (error) toast.error("Failed to update client details");     else { toast.success("Client updated — enrichment will refresh automatically"); setEditing(false); onUpdated(name.trim(), phone.trim()); }   };   if (!editing) return (     <button onClick={e => { e.stopPropagation(); setName(currentName); setPhone(currentPhone ?? ""); setEditing(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors border border-white/[0.06]" title="Edit client name / phone">       <Pencil className="w-3 h-3" /> Edit details     </button>   );   return (     <div className="flex flex-col gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]" onClick={e => e.stopPropagation()}>       <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-white/30">Edit Client Details</p>       <div className="flex flex-col gap-1.5">         <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Full name" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />         <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone (e.g. 0821234567)" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />       </div>       <p className="text-[10px] text-amber-400/70 leading-relaxed">💡 Updating the name to match bookings (e.g. "Amy" → "Amy Damons") fixes live date matching automatically.</p>       <div className="flex gap-2">         <button onClick={save} disabled={saving || !name.trim()} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-[11px] font-semibold text-emerald-400 hover:bg-emerald-500/25 transition-colors disabled:opacity-40">           {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} {saving ? "Saving…" : "Save"}         </button>         <button onClick={() => setEditing(false)} className="px-2.5 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>       </div>     </div>   ); };  // ─── EnrollModal ───
...
const ClientRow = ({
  ...
  // ─── InlioptimisticStatus, onOptimisticStatus, onUnregister,onOptimisticStatus: (s: string) => void; onUnregister: () => void;neClientEditor ─── const InlineClientEditor = ({ rowId, currentName, currentPhone, tenantId, onUpdated }: {   rowId: string; currentName: string; currentPhone: string | null; tenantId: string; onUpdated: (newName: string, newPhone: string) => void; }) => {   const [editing, setEditing] = useState(false);   const [name, setName] = useState(currentName);   const [phone, setPhone] = useState(currentPhone ?? "");   const [saving, setSaving] = useState(false);   const save = async () => {     if (!name.trim()) return;     setSaving(true);     const { error } = await supabase.from("loyalty_tracker").update({ client_name: name.trim(), phone: phone.trim() || null, updated_at: new Date().toISOString() }).eq("id", rowId).eq("tenant_id", tenantId);     setSaving(false);     if (error) toast.error("Failed to update client details");     else { toast.success("Client updated — enrichment will refresh automatically"); setEditing(false); onUpdated(name.trim(), phone.trim()); }   };   if (!editing) return (     <button onClick={e => { e.stopPropagation(); setName(currentName); setPhone(currentPhone ?? ""); setEditing(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors border border-white/[0.06]" title="Edit client name / phone">       <Pencil className="w-3 h-3" /> Edit details     </button>   );   return (     <div className="flex flex-col gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]" onClick={e => e.stopPropagation()}>       <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-white/30">Edit Client Details</p>       <div className="flex flex-col gap-1.5">         <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Full name" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />         <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone (e.g. 0821234567)" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />       </div>       <p className="text-[10px] text-amber-400/70 leading-relaxed">💡 Updating the name to match bookings (e.g. "Amy" → "Amy Damons") fixes live date matching automatically.</p>       <div className="flex gap-2">         <button onClick={save} disabled={saving || !name.trim()} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-[11px] font-semibold text-emerald-400 hover:bg-emerald-500/25 transition-colors disabled:opacity-40">           {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} {saving ? "Saving…" : "Save"}         </button>         <button onClick={() => setEditing(false)} className="px-2.5 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>       </div>     </div>   ); };  // ─── EnrollModal ───
}: {
        ...
        {expanded && (
          ...
          >
            ...
            <div className="px-3 pb-3 border-t border-white/[0.04] pt-2.5 flex flex-col gap-2">
              ...
              // ─── Inli<InlineNotesEditor rowId={r.id} current={r.notes} tenantId={tenantId} onUpdated={onUpdated} /><InlineClientEditor rowId={r.id} currentName={r.client_name} currentPhone={r.phone} tenantId={tenantId} onUpdated={(newName, newPhone) => { onUpdated(); }} /><UnregisterButton rowId={r.id} clientName={r.client_name} tenantId={tenantId} onUnregister={onUnregister} />optimisticStatus, onOptimisticStatus, onUnregister,onOptimisticStatus: (s: string) => void; onUnregister: () => void;neClientEditor ─── const InlineClientEditor = ({ rowId, currentName, currentPhone, tenantId, onUpdated }: {   rowId: string; currentName: string; currentPhone: string | null; tenantId: string; onUpdated: (newName: string, newPhone: string) => void; }) => {   const [editing, setEditing] = useState(false);   const [name, setName] = useState(currentName);   const [phone, setPhone] = useState(currentPhone ?? "");   const [saving, setSaving] = useState(false);   const save = async () => {     if (!name.trim()) return;     setSaving(true);     const { error } = await supabase.from("loyalty_tracker").update({ client_name: name.trim(), phone: phone.trim() || null, updated_at: new Date().toISOString() }).eq("id", rowId).eq("tenant_id", tenantId);     setSaving(false);     if (error) toast.error("Failed to update client details");     else { toast.success("Client updated — enrichment will refresh automatically"); setEditing(false); onUpdated(name.trim(), phone.trim()); }   };   if (!editing) return (     <button onClick={e => { e.stopPropagation(); setName(currentName); setPhone(currentPhone ?? ""); setEditing(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors border border-white/[0.06]" title="Edit client name / phone">       <Pencil className="w-3 h-3" /> Edit details     </button>   );   return (     <div className="flex flex-col gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]" onClick={e => e.stopPropagation()}>       <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-white/30">Edit Client Details</p>       <div className="flex flex-col gap-1.5">         <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Full name" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />         <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone (e.g. 0821234567)" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />       </div>       <p className="text-[10px] text-amber-400/70 leading-relaxed">💡 Updating the name to match bookings (e.g. "Amy" → "Amy Damons") fixes live date matching automatically.</p>       <div className="flex gap-2">         <button onClick={save} disabled={saving || !name.trim()} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-[11px] font-semibold text-emerald-400 hover:bg-emerald-500/25 transition-colors disabled:opacity-40">           {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} {saving ? "Saving…" : "Save"}         </button>         <button onClick={() => setEditing(false)} className="px-2.5 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>       </div>     </div>   ); };  // ─── EnrollModal ───
            </div>
          </motion.div>
        )}
      </AnimatePresence>
              ...
              <div className="flex flex-col gap-2">
                {filteredRows.map((r, i) => (
                  <ClientRow
                    ...
                    // ─── InlionOptimisticStatus={(s) => setOptimisticStatuses(prev => ({ ...prev, [r.id]: s }))} onUnregister={() => { qc.invalidateQueries({ queryKey: ["loyalty", tenantId] }); }}// ─── UnregisterButton ─── const UnregisterButton = ({ rowId, clientName, tenantId, onUnregister }: { rowId: string; clientName: string; tenantId: string; onUnregister: () => void; }) => {   const [confirm, setConfirm] = useState(false);   const [deleting, setDeleting] = useState(false);   const doDelete = async () => {     setDeleting(true);     const { error } = await supabase.from("loyalty_tracker").delete().eq("id", rowId).eq("tenant_id", tenantId);     if (error) { toast.error("Failed to remove client"); setDeleting(false); setConfirm(false); }     else { toast.success(`${clientName} removed from loyalty tracker`); onUnregister(); }   };   if (!confirm) return (     <button onClick={e => { e.stopPropagation(); setConfirm(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-red-400/50 hover:text-red-400 hover:bg-red-500/[0.07] transition-colors border border-red-500/[0.12]" title="Remove from loyalty tracker">       <Trash2 className="w-3 h-3" /> Unregister     </button>   );   return (     <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-500/[0.07] border border-red-500/20" onClick={e => e.stopPropagation()}>       <p className="text-[10px] text-red-400/80 flex-1">Remove <span className="font-semibold">{clientName}</span> from loyalty tracker?</p>       <button onClick={doDelete} disabled={deleting} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-[11px] font-semibold text-red-400 hover:bg-red-500/30 transition-colors disabled:opacity-40">         {deleting ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />} {deleting ? "Removing…" : "Yes, remove"}       </button>       <button onClick={() => setConfirm(false)} className="px-2 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>     </div>   ); };  // ─── EnrollModal ───<InlineNotesEditor rowId={r.id} current={r.notes} tenantId={tenantId} onUpdated={onUpdated} /><InlineClientEditor rowId={r.id} currentName={r.client_name} currentPhone={r.phone} tenantId={tenantId} onUpdated={(newName, newPhone) => { onUpdated(); }} /><UnregisterButton rowId={r.id} clientName={r.client_name} tenantId={tenantId} onUnregister={onUnregister} />optimisticStatus, onOptimisticStatus, onUnregister,onOptimisticStatus: (s: string) => void; onUnregister: () => void;neClientEditor ─── const InlineClientEditor = ({ rowId, currentName, currentPhone, tenantId, onUpdated }: {   rowId: string; currentName: string; currentPhone: string | null; tenantId: string; onUpdated: (newName: string, newPhone: string) => void; }) => {   const [editing, setEditing] = useState(false);   const [name, setName] = useState(currentName);   const [phone, setPhone] = useState(currentPhone ?? "");   const [saving, setSaving] = useState(false);   const save = async () => {     if (!name.trim()) return;     setSaving(true);     const { error } = await supabase.from("loyalty_tracker").update({ client_name: name.trim(), phone: phone.trim() || null, updated_at: new Date().toISOString() }).eq("id", rowId).eq("tenant_id", tenantId);     setSaving(false);     if (error) toast.error("Failed to update client details");     else { toast.success("Client updated — enrichment will refresh automatically"); setEditing(false); onUpdated(name.trim(), phone.trim()); }   };   if (!editing) return (     <button onClick={e => { e.stopPropagation(); setName(currentName); setPhone(currentPhone ?? ""); setEditing(true); }} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-medium text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors border border-white/[0.06]" title="Edit client name / phone">       <Pencil className="w-3 h-3" /> Edit details     </button>   );   return (     <div className="flex flex-col gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]" onClick={e => e.stopPropagation()}>       <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-white/30">Edit Client Details</p>       <div className="flex flex-col gap-1.5">         <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Full name" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />         <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone (e.g. 0821234567)" className="text-[11px] bg-white/[0.06] border border-white/[0.12] rounded-lg px-2 py-1.5 text-white/80 focus:outline-none focus:border-emerald-400/40 w-full" />       </div>       <p className="text-[10px] text-amber-400/70 leading-relaxed">💡 Updating the name to match bookings (e.g. "Amy" → "Amy Damons") fixes live date matching automatically.</p>       <div className="flex gap-2">         <button onClick={save} disabled={saving || !name.trim()} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-[11px] font-semibold text-emerald-400 hover:bg-emerald-500/25 transition-colors disabled:opacity-40">           {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} {saving ? "Saving…" : "Save"}         </button>         <button onClick={() => setEditing(false)} className="px-2.5 py-1 rounded-lg text-[11px] text-white/30 hover:text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>       </div>     </div>   ); };  // ─── EnrollModal ───
                  />
                ))}
              </div>
            )}
