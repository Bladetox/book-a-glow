...
// Views that appear before the divider (operational views)
const DIVIDER_BEFORE = "Client Management";
        ...
        <nav className="flex flex-col gap-0.5 px-2 py-3 flex-1">
          {views.map((view) => {
            ...
            return (
              <>
                {view === DIVIDER_BEFORE && (
                  <div key={`divider-${view}`} className="mx-2 my-1.5 h-px bg-white/[0.06]" />
                )}
                <button
                  key={view}
