...
const ReviewStep = ({ booking }: ReviewStepProps) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // If no deposit required, booking is confirmed — go to success page directly
      if (!config.requiresDeposit || deposit === 0) {
        window.location.href = `/booking/success?id=${bookingId}&tenant=${tenantId}&type=confirmed`;
        return;
      }

      // Deposit required — initiate Yoco checkout
      ...
      if (!res.ok) throw new Error(checkoutData.error || "Failed to create checkout");
