...
/** Format a rand value with en-ZA locale, no decimals for whole numbers */
...
export function useNextyInsights() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Fires when top service earns >2x more per minute than bottom service.
        ...
        if (topRpm > bottomRpm * 2) {
          ...
          insights.push({
            ...
            message:
              ...
              `That is a ${multiplier}x difference for the same chair time. ` +
        ...
        if (pct < 75) {
          insights.push({
            ...
            message:
              ...
              `${slowest.booking_count} booking${slowest.booking_count === 1 ? "" : "s"} compared to ` +
      ...
      // -- 3. No-Show Audit ----------------------------------------------------
      // Only fires when no_show_rate > 5%. No advisory shown for clean records.
        ...
        if (rate > 5) {
          insights.push({
            ...
            message:
              ...
              `Your no-show rate is ${rate.toFixed(1)}%, meaning roughly 1 in every ${Math.round(100 / rate)} appointments leaves without revenue. ` +
      ...
      if (loyaltyGap && loyaltyGap[0] && loyaltyGap[0].ghost_regular_count > 0) {
        ...
        insights.push({
          ...
          message:
            ...
            `At an average of ${formatRand(Math.round(avgSpend))} per visit, enrolling ` +
            `${count === 1 ? "them" : `all ${count}`} secures an estimated ` +
            `${formatRand(Math.round(annualRevenue))} in anchored annual revenue.`,
      ...
      // Fires only when rebooking rate < 55% with at least 3 first-time clients.
        ...
        if (rate < 55) {
          insights.push({
            ...
            message:
              ...
              `The other ${total - rebooked} client${total - rebooked === 1 ? "" : "s"} did not return. ` +
              `A follow-up message 48 hours after a first visit is the most reliable way to recover those bookings.`,
            actionLabel: "Review Client Flow",
            actionView: "ClientManagement",
          });
        }
        // Rate >= 55%: healthy, no card rendered.
      }
