-- ============================================================
-- Hotfix: Restore backwards-compatible signature for
-- create_booking_with_consultation.
--
-- CONTEXT: The previous migration (20260320_security_remove_frontend_amounts)
-- removed p_total_amount, p_deposit_amount, p_tenant_id from the function.
-- The live Vercel deployment (main branch) had not yet been redeployed and
-- was still sending those params → PGRST202 → bookings were broken.
--
-- FIX: Re-add the three legacy params as optional (DEFAULT NULL) so the
-- old and new frontends both work. The function body IGNORES them entirely
-- and always computes pricing server-side via calculate_booking_price.
-- This preserves the security fix while unblocking the live app.
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.create_booking_with_consultation(
  ...
  -- Legacy params: accepted for backwards compatibility, intentionally IGNORED.
  -- Pricing is always server-side via calculate_booking_price.
...
BEGIN
  -- Resolve tenant_id from first service (p_tenant_id param is ignored)
